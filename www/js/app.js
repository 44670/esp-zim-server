...
define(['jquery', 'zimArchiveLoader', 'uiUtil', 'settingsStore', 'abstractFilesystemAccess'],
    function ($, zimArchiveLoader, uiUtil, settingsStore, abstractFilesystemAccess) {
        function readFileSliceWS(offset, length) {
            console.log('r', offset, length);
            return new Promise(function (resolve, reject) {
                var xhr = new XMLHttpRequest();
                xhr.open('GET', "/zim?" + offset + "," + length, true);
                xhr.responseType = 'arraybuffer';
                xhr.onreadystatechange = function (e) {
                    if (xhr.readyState == 4) {
                        console.log('resp', xhr.status);
                        if (xhr.status == 200) {
        ...
        readFileSliceWS(0, 80).then((buf) => {
            var dv = new DataView(buf)
            var v1 = dv.getUint32(72, true)
            var v2 = dv.getUint32(76, true)
            var checksumPos = v1 + 4294967296 * v2  //This points always 16 bytes before the end of the file. 
            console.log('checksumPos', checksumPos)
            var fileObj = {
                name: 'ws.zim',
                size: checksumPos + 16
            }
            fileObj.slice = function (start, end) {
                return {
                    arrayBuffer: function () {
                        return readFileSliceWS(start, end - start)
                    }
                }
            }
            setLocalArchiveFromFileList([fileObj])
        })
        /**
